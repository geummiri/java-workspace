 ...
 * - java.lang 패키지에 있는 클래스와 인터페이스는 import 없이 사용할 수 있당
 * 
 */
...
//		a.method5();
	...
	public void method1() {
		...
		System.out.println("str3 == str4 : " + (str3 == str4));//false 힙에서 동일한 메모리 주소를 참조하지 않기 때문에
		...
		//hashCode() 메서드는 String 클래스에서 오버라이딩 재정의 되어있어서값만 비교를 해서 같았음
		...
		public void method2() {
			...
			System.out.println("ch : " + ch);//ch : l(소문자 l)
			...
			index = str.indexOf('?');
			System.out.println(index); //-1 반환됨
			System.out.println();
			...
			//7. trim() : String
			//		-문자열의 앞뒤 공백을 제거한 새로운 문자열을 생서해서 리턴
			str = "               Hello, World!             ";
	 ...
	 * 	- StringBuilder와 StringBuffer의 차이점 : 동기화가 되는냐 안되느냐
	...
	public void method3() {
		...
		//StringBuilder클래스의 생성자에 정수값을 전달해주면 해당 크기의 버퍼가 생성
		StringBuilder sb2 = new StringBuilder(30);
		System.out.println("sb2의 문자열 길이 : " + sb2.length()); //0
		System.out.println("sb2의 버퍼의 크기 : " + sb2.capacity()); //30
		
		//StringBuilder 클래스의 생성자에 문자열을 전달해주면 문자열 크기 + 16 만큼의 버퍼가 생성됨
	...
	public void method4() {
		...
		//5.메서드체이닝 : 메서드를 이어서 쓸 수 있다.
		StringBuilder sb2 = new StringBuilder("Java Program");
		//IPA avaJ
		sb2.delete(4,12).insert(4, " API").reverse();
