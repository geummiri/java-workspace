 ...
 * [표현법]
 * 
 * [접근제어자_ex.public] class 자식클래스 extends 부모클래스 {
 * 
 * 			//멤버변수
 * 			//생성자
 *			//메서드
 ...
 *  -> object클래스에서 제공하는 메서드들을 오버라이딩하며 메서드 재정의가 가능
...
//		super.pCode = pCode; //부모가 defalut -같은 패키지에 있어야만 직접 접근이 가능
//		super.name = name; //부모가 protected - 상속 관계일 때 자식 객체에서 직접 접근 가능
//		super.price = price; //public 어디서든 접근이 가능
		
		/*
		 * 2.부모의 Setter 메서드를 호출해서 초기화
		 *  - 부모 클래스에서 setter 메서드를 제공하면 자식 객체에서 부모의 setter 메서드를 통해서 필드의 초기화 가능
		 * 
		 */
		
//		super.setBrand(brand); //부모 클래스 객체 필드 초기화 가눙
//		this.setpCode(pCode);
//		setName(name);
//		setPrice(price);
